...
int main(int argc, char *argv[])
{
	...
	cudaStream_t mult_stream[STREAM_NUM];
	...
	for (i = 0; i < task; i++)
	{
		checkCudaErrors(cudaMemcpyAsync(A_dev[i], A[i], MSIZE * sizeof(float), cudaMemcpyHostToDevice, mult_stream[i % STREAM_NUM]));
		checkCudaErrors(cudaMemcpyAsync(B_dev[i], B[i], MSIZE * sizeof(float), cudaMemcpyHostToDevice, mult_stream[i % STREAM_NUM]));
	}
	...
	for (i = 0; i < task; i++)
	{
		mult_gpu<<<1, TD_NUM, 0, mult_stream[i % STREAM_NUM]>>>(A_dev[i], B_dev[i], C_dev[i], mCols, TD_NUM);
	}
	checkCudaErrors(cudaDeviceSynchronize());
	end_timer = my_timer();
	...
	for (i = 0; i < task; i++)
		checkCudaErrors(cudaMemcpyAsync(C[i], C_dev[i], MSIZE * sizeof(float), cudaMemcpyDeviceToHost, mult_stream[i % STREAM_NUM]));
	checkCudaErrors(cudaDeviceSynchronize());
